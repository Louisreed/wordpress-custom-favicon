...
function custom_favicon_options() {
    ...
    if ( isset( $_POST['custom_favicon_form_submitted'] ) ) {
        ...
        $custom_favicon_url = sanitize_text_field( $_POST['custom_favicon'] );
        update_option( 'custom_favicon_url', $custom_favicon_url );
        ?>
        <div class="updated"><p><strong><?php _e( 'Changes saved.', 'custom-favicon' ); ?></strong></p></div>
        <?php
    }
...
?>
<div class="wrap">
    <h2><?php _e( 'Custom Favicon', 'custom-favicon' ); ?></h2>
    <form name="custom_favicon_form" method="post" action="">
        <?php wp_nonce_field( 'custom_favicon_form_action', 'custom_favicon_form_nonce_field' ); ?>
        <table class="form-table">
            <tr valign="top">
                <th scope="row"><label for="custom_favicon"><?php _e( 'Favicon URL', 'custom-favicon' ); ?></label></th>
                <td>
                    <input type="text" name="custom_favicon" id="custom_favicon" value="<?php echo esc_url( get_option( 'custom_favicon_url' ) ); ?>" size="60" />
                    <input id="upload_image_button" type="button" class="button" value="Upload Image" />
                    <p class="description"><?php _e( 'Enter the URL for the favicon image, or use the upload button to select an image from the media library.', 'custom-favicon' ); ?></p>
                    <?php
            ...
            <p class="submit">
                <input type="submit" name="custom_favicon_form_submitted" class="button-primary" value="<?php _e( 'Save Changes', 'custom-favicon' ); ?>" />
            </p>
        </form>
    </div>
    <?php
...
function custom_favicon() {
    // Get the custom favicon URL
    $custom_favicon_url = esc_url( get_option( 'custom_favicon_url' ) );

    // If a custom favicon URL has been entered, use it
    if ( ! empty( $custom_favicon_url ) ) {
        echo '<link rel="shortcut icon" href="' . $custom_favicon_url . '" />';
    }
}
